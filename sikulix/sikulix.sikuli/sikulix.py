...
while True:
        ...
        if m1:
            ...
            #result = subprocess.check_output('python D://lol_project/run_node/sikulix/python/darwin.py %s %s'%(img1,img2), shell=True)
            print(result)
            type(result)
            if pick_mode == 1:
                sleep(4)
        ...
        if exists("1506065189732.png",0):
            ...
            Debug.user( u"收竿費時 %s" % (t2-t1) )
            type(Key.SPACE)
            sleep(1.60+0.1)
            type(Key.SPACE)
            is_shooting = 0
            count += 1
            print("釣魚次數: %d    招標次數: %d" % (count,count2))
            is_fishing = 5
            continue
        #print_time("f3")
        ...
        if exists("1508519043012.png",0):
            if exists("1506871959078.png",0):
                myClick(exists("1508600299846.png",0))
            if store == 1:
                #myClick(exists("1508688288467.png",0))
                #myClick(exists("1508863060999.png",0))
                myClick(exists("1509104285011.png",0))
                #myClick(exists("1509352755397.png",0))
                store = 1
            elif store == 2:
